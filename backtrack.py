...
def parse_paths_to_rgb(paths_taken, field):
    ...
    rgb_field = np.zeros(shape=(len(field), len(field[0]), 3), dtype=int)
    for i in range(0, len(paths_taken)):
        if field[paths_taken[i]] == MARKER or field[paths_taken[i]] == FREE:
            rgb_field[paths_taken[i]] = rgb_field[paths_taken[i]] + 200
...
#maze = maze_gen.getMaze(10, 10, 15)
...
plt.ion()
fig, ax = plt.subplots()
text = ax.text(.01, .99, str(num_solves), color="white")
image = ax.imshow(parse_field_to_rgb(maze))
plt.show()


def print_to_display(path, num):
    data = parse_paths_to_rgb(path, maze)
    image.set_data(data)
...
def solve_maze(row, column, path):
    ...
    if is_escape(row, column):
        ...
        if(num_solves % 1000) == 0:
            print_to_display(path, num_solves)
        num_solves += 1
...
plt.imshow(parse_paths_to_rgb(best, maze))
plt.show()
